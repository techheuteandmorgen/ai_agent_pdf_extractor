...
def validate_and_calculate_premiums(data):
    """
    Validate and ensure premiums are consistent and accurate.
    """
    try:
        # Extract premiums
        ...
        # Ensure NET_PREMIUM is directly extracted
        if "NET_PREMIUM" in data and net_premium != 0:
            data["NET_PREMIUM"] = net_premium
        else:
            # Recalculate NET_PREMIUM if missing
            calculated_net = od_premium + tp_premium
            print(f"Recalculating NET_PREMIUM: {calculated_net}")
            data["NET_PREMIUM"] = calculated_net

        # Ensure TOTAL_PREMIUM matches extracted value
        if "TOTAL_PREMIUM" in data and total_premium != 0:
            data["TOTAL_PREMIUM"] = total_premium
        else:
            print(f"Recalculating TOTAL_PREMIUM to match NET_PREMIUM: {data['NET_PREMIUM']}")
...
def process_pdf(pdf_path, user_id=None):
    ...
    try:
        ...
        # Debugging: Print final structured data
